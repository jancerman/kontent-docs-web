...
(function () {
  ...
  var createTableOfContents = function createTableOfContents() {
    ...
    headings.forEach(function (item) {
      ...
      tableOfContents += "<li><a href=\"#".concat(item.getAttribute('id'), "\">").concat(window.helper.encodeHTMLEntities(item.textContent), "</a></li>");
...
(function () {
  var FILTER_ACTIVE_CLASSNAME = 'filter__item--active';

  var itemInteractionServices = function itemInteractionServices(target, filterElems) {
    ...
    var activeFilterItems = Array.prototype.slice.call(filterItems).filter(function (elem) {
      return elem.classList.contains(FILTER_ACTIVE_CLASSNAME);
    });
    var activeFilterCodenames = [];
    ...
    for (var _i = 0; _i < filterElems.length; _i++) {
      ...
      if (activeFilterCodenames.length) {
        ...
        for (var j = 0; j < activeFilterCodenames.length; j++) {
  ...
  var itemInteractionChanges = function itemInteractionChanges(target, filterElems) {
    var filterItems = helper.findAncestor(target, '.filter__list').querySelectorAll('.filter__item');
    ...
    var activeFilterCodename = activeFilterItems.length ? activeFilterItems[0].getAttribute('data-breaking-change-option') : '';

    for (var _i2 = 0; _i2 < filterElems.length; _i2++) {
      filterElems[_i2].classList.remove('breaking-hidden');

      if (activeFilterCodename === 'true') {
        var isBreaking = filterElems[_i2].getAttribute('data-breaking-change');

        if (isBreaking !== 'true') {
          filterElems[_i2].classList.add('breaking-hidden');
        }
      }
    }
  };
  ...
  var filterInteractions = function filterInteractions() {
    ...
    var breakingChangeElems = document.querySelectorAll('[data-breaking-change]');
    document.querySelector('body').addEventListener('click', function (e) {
      if (e.target) {
...
(function () {
  ...
  var asideElems = document.querySelectorAll('[data-aside]');

  var moveToAside = function moveToAside() {
    for (var i = 0; i < asideElems.length; i++) {
      asideContainer.appendChild(asideElems[i]);
    }
  };

  var moveFromAside = function moveFromAside() {
    if (!asideContainer.childNodes.length) {
      return;
    }

    for (var i = 0; i < asideElems.length; i++) {
      var codename = asideElems[i].getAttribute('data-aside');
      var wrapper = document.querySelector("[data-aside-container=\"".concat(codename, "\"]"));
      wrapper.appendChild(asideElems[i]);
    }
  };

  var enableDisableAside = function enableDisableAside(mql) {
    if (mql.matches) {
      moveFromAside();
    } else {
      moveToAside();
