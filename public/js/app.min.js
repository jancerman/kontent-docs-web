...
(function () {
  ...
  var fixSubNav = function fixSubNav() {
    ...
    if (viewportWidth >= 768 && subNavigation) {
      ...
      var isBottom = window.innerHeight + window.pageYOffset + window.helper.outerHeight(document.querySelector('.footer')) >= document.body.offsetHeight;
...
(function () {
  ...
  var hidePlaformInContentChunk = function hidePlaformInContentChunk(item, languageSelector) {
    var chunkParent = window.helper.findAncestor(item, '[data-platform-chunk]');

    if (chunkParent) {
      var languageSelectorItems = languageSelector.querySelectorAll('.language-selector__link');
      var chunkPlatforms = chunkParent.getAttribute('data-platform-chunk').split('|');
      languageSelectorItems.forEach(function (elem) {
        var elemParent = window.helper.findAncestor(elem, '.language-selector__item');
  ...
  var showAllPlatformsInContentChunk = function showAllPlatformsInContentChunk(languageSelector) {
    var languageSelectorItems = languageSelector.querySelectorAll('.language-selector__link');
    languageSelectorItems.forEach(function (elem) {
      var elemParent = window.helper.findAncestor(elem, '.language-selector__item');
      elemParent.style.display = 'block';
  ...
  var cloneLanguageSelectorToCodeBlocks = function cloneLanguageSelectorToCodeBlocks() {
    ...
    if (languageSelector && languageSelector.querySelector('.language-selector__list:not(.language-selector__list--static)') && languageSelector.querySelector('.language-selector__list').childNodes.length > 1) {
      ...
      var codeBlocks = document.querySelectorAll('*:not([data-platform-code]) + [data-platform-code]:not([data-platform-code=""]), [data-platform-code]:first-child:not([data-platform-code=""])');
      ...
      codeBlocks.forEach(function (item) {
        ...
        var clonedSelector = item.parentNode.insertBefore(languageSelector, item);
  ...
  var handleEmptyPlatforms = function handleEmptyPlatforms() {
    ...
    codeBlocks.forEach(function (block) {
      var availablePlatforms = _toConsumableArray(block.querySelectorAll('[data-platform]')).map(function (item) {
        return item.getAttribute('data-platform');
      });

      var availableCodeBlocks = _toConsumableArray(block.querySelectorAll('[data-platform-code]')).map(function (item) {
      ...
      availablePlatforms.forEach(function (platform) {
        ...
        block.querySelector("[data-platform=\"".concat(platform, "\"]")).classList.add('language-selector__empty');
      });
...
(function () {
  ...
  var createAnchors = function createAnchors() {
    ...
    headings.forEach(function (item) {
      ...
      item.setAttribute('id', "a-".concat(anchorName).concat(anchorNameCount > 1 ? "-".concat(anchorNameCount) : ''));
      item.innerHTML = "".concat(item.innerHTML, "<span class=\"anchor-copy\" aria-hidden=\"true\"><span class=\"anchor-copy__tooltip\"></span></span>");
    });
  }; // Make all icons copy the headings URL to clipboard and show appropriate message in tooltip


  var copyAnchorClipboard = function copyAnchorClipboard() {
    var anchors = document.querySelectorAll('.anchor-copy');
    anchors.forEach(function (item) {
      item.addEventListener('click', function () {
        var hash = item.parentElement.getAttribute('id');
        var url = window.location.href.split('#')[0];
        window.helper.copyToClipboard("".concat(url, "#").concat(hash));
        var tooltip = item.querySelector('.anchor-copy__tooltip');
  ...
  var createTableOfContents = function createTableOfContents() {
    var headings = articleContent.querySelectorAll('h2:not(.table-of-contents__heading):not(.table-of-contents__whatsnext):not(.feedback__heading)');
    var tableOfContents = '';
    ...
    headings.forEach(function (item) {
      ...
      tableOfContents += "<li><a href=\"#".concat(item.getAttribute('id'), "\">").concat(item.innerHTML, "</a></li>");
...
(function () {
  ...
  var handleFixed = function handleFixed() {
    ...
    if (viewportWidth >= 1150 && selector) {
      var topOffset = (window.pageYOffset || document.scrollTop) - (document.clientTop || 0) || 0;
      var main = document.querySelector('.article__content .table-of-contents + *') || document.querySelector('.article__filter');
      var isTop = topOffset <= (main.getBoundingClientRect().top || 0) + main.offsetHeight + (window.scrollY || document.documentElement.scrollTop);
...
(function () {
  var createSections = function createSections(content) {
    ...
    for (var _i = 0; _i < sections.length; _i++) {
      ...
      var nextSection = sections[_i + 1];

      if (nextSection) {
        var nextLink = document.createElement('a');
        nextLink.setAttribute('href', "#".concat(nextSection.getAttribute('data-display-mode-id')));
        ...
        sections[_i].appendChild(nextLink);
      }
